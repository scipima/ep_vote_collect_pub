...
process_vote_day <- function(votes_raw = vote_list_tmp[["MTG-PL-2020-01-29"]]) {
...
#     votes_raw = vote_list_tmp[[i_vote]]
    ...
    cols_character <- c("id", "activity_date", "activity_id", "activity_start_date",
                        ...
                        "forms_part_of", "decided_on_a_part_of_a_realization_of")
...
###--------------------------------------------------------------------------###
# test ------------------------------------------------------------------------#
# pp1=process_vote_day()
p = lapply(X = vote_list_tmp, FUN = function(x) process_vote_day(x))
p1 = data.table::rbindlist(p, fill = TRUE, use.names = TRUE)
apply(p1, MARGIN = 2, function(x) length(na.omit(unique(x))))
